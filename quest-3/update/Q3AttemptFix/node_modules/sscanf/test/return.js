...
describe('scanf', function() {
  describe('#Return type', function() {
    ...
    it('[%d %f %s %x %o] should return a json', function(done) {
      let result = sscanf('12 3.1415926 hello 1F 10', '%d %f %s %x %o', 'integer', 'float', 'string', 'hex', 'octal');
    ...
    it('[%d %s %s %s %s %S] should return a json', function(done) {
      let result = sscanf('   14   ??  Ss     0:07.59 /usr/sbin/securityd -i -j -k -l', '%d %s %s %s %s %S', 'pid', 'tty', 'stat', 'time', 'exec', 'param');
      should.deepEqual(result, {
        pid: 14,
        tty: '??',
        stat: 'Ss',
    ...
    it('a[%x]', function(done) {
      let str = "a[1A]";
      let result = sscanf(str, 'a[%x]', 'address');
      should.deepEqual(result, {
        address: 26
      });
      done();
    ...
    it('a(%s)', function(done) {
      ...
      let result = sscanf(str, 'a(%s)', 'text');
      should.deepEqual(result, {
        text: 'hello world'
      });
      done();
    ...
    it('|%s|%s|%s', function(done) {
      let str = '|test|hello|string';
      let result = sscanf(str, '|%s|%s|%s');
      should.deepEqual(result, ['test', 'hello', 'string']);
      done();
    });

    it('|%s|%s|%s|sla|%s|%d', function(done) {
      let str = '|Server1|CTRL_SLA|Intranet|sla|2015-06-15\\ 16:25:28|100';
      let result = sscanf(str, '|%s|%s|%s|sla|%s|%d');
