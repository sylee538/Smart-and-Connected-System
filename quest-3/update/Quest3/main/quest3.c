...
int water_flag = 0; // if time for water, base setting is 60 min
int alert_control = 0;
int on_control = 1;
int water_control = 0;
...
static void task_vib() {
        ...
        while (1) {
                ...
                if ((save != 0)  && (currentState == saveLastState) && (saveDebounceTimeout > DEBOUNCETIME )) {
                        ...
                        look_flag = (alert_control == 1 || water_flag == 1) ? 1 : 0;
                        printf("%d\n", look_flag);
...
static void control(int ins) {
        switch (ins) {
        case 1: water_control = 1;
                break;
        case 2: water_control = 0;
                break;
        case 3: alert_control = 1;
                break;
        }
        printf("Alert: %d Water: %d\n", alert_control, water_control);
        printf("WF: %d, LF: %d\n", water_flag, look_flag);
}
...
void app_main() {
        ...
        xTaskCreate(task_alert, "task_alert",1024*2, NULL, MAX_PRIO + 1, NULL);
